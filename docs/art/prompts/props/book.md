...
`book.png`, около 128×96. Ведёт в дневник.
...
```
A closed leather-bound journal seen from the front cover, dark brown leather, brass
corner fittings, a leather strap clasp on the right side.
...
- переплёт читается закрытым: раскрытая книга на таком размере превращается в пятно;
- уголки латунные, иначе книга сольётся с сундуком по силуэту.
...
В [`incoming/`](../../../../incoming/) под именем `book.png`.
