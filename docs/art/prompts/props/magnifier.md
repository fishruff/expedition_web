...
`magnifier.png`, около 96×64. Декор.
...
```
A magnifying glass with a brass rim and a wooden handle, oriented diagonally, handle to
the lower left.
...
- стекло светлее фона, но не белое: белое пятно перетянет внимание со всего стола;
- ручка деревянная — иначе лупа сольётся с компасом по цвету.
...
В [`incoming/`](../../../../incoming/) под именем `magnifier.png`.
