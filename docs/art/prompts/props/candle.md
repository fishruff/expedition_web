...
`candle.png`, около 48×96. Декор: второй огонёк на столе.
...
```
A short candle stub with a small warm flame, standing in a simple brass holder with a
ring handle.
...
- огонёк мельче, чем у лампы: два одинаковых источника спорят друг с другом;
- подтёки воска — по одной стороне, не симметрично.
...
В [`incoming/`](../../../../incoming/) под именем `candle.png`.
