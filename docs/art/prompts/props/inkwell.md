...
```
A glass inkwell with a brass rim, a white feather quill standing in it at a slight angle.
...
background. Hand-drawn pixel art, about 100 pixels tall, chunky visible pixels, no
...
- перо светлое — единственное светлое пятно в предмете, иначе оно потеряется;
- чернила внутри тёмные, но не чёрные.
...
В [`incoming/`](../../../../incoming/) под именем `inkwell.png`.
