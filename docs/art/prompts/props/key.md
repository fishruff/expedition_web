...
`key.png`, около 96×48. Декор: намёк на запертое.
...
```
A large old iron key with an ornate bow, oriented horizontally, teeth to the right.
...
- железо, а не латунь: ключ должен отличаться по цвету от остального стола;
- головка узорная — по ней ключ узнают в мелком размере.
...
В [`incoming/`](../../../../incoming/) под именем `key.png`.
