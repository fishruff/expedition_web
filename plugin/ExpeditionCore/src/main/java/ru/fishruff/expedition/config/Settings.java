...
public record Settings(
     ...
     *
     * Правило то же, что у приёмника (`api/src/main.ts`): печатные ASCII без
     * пробела, то есть от `!` до `~`. Проверки две, потому что стороны написаны
     * на разных языках, но список правил обязан быть один. Раньше он разъезжался:
     * плагин пропускал пробел и управляющие знаки, приёмник — нет, и обе стороны
     * просто падали по-разному на одном и том же значении.
     */
        ...
        for (int i = 0; i < key.length(); i++) {
            ...
            if (symbol < 0x21 || symbol > 0x7e) {
                throw new IllegalArgumentException(
                        "EXPEDITION_KEY должен быть из латиницы, цифр и знаков без пробелов: "
                                + "заголовки HTTP не переносят ни кириллицу, ни пробелы");
            }
        }
    }
}
